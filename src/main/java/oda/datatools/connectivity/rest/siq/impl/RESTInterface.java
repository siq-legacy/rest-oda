...
public class RESTInterface {
	...
	public void setDataMappingList(Vector<Map<String, String>> dataMappingList) {
		this.dataMappingList = dataMappingList;
	}

	public void setColumnMappingList(Vector<ColumnNameMapping> columnMappingList) {
		this.columnMappingList = columnMappingList;
	}

	public String login_app_url(String url) {
		url = url.substring(0, url.indexOf('/', 8))
				+ RESTConstants.LOGIN_URL_APPSTACK;
		return url;
	...
	private int fillColumnMappings(String response,
			ColumnNameMapping columnNameMapping) throws JSONException {

		JSONArray jsonresourceobj = null;
		JSONObject jb=RESTJsonExtract.parse(response);
		...
		for (int j = 0; j < jsonresourceobj.size(); j++) {
			...
			this.columnMapping.put(obj1.get(columnNameMapping.getSourceKey()),
					obj1.get(columnNameMapping.getSourceValue()));
		}
		return jsonresourceobj.size();
	}

	public RESTList executeQuery(RESTConnection connection) throws OdaException, JSONException {
		...
		try {
			...
			throw new OdaException(
					"Connection to datasource failed ,pls check the datasource again "
							+ e1.getMessage());
		...
		while (position < query.length) {
			...
			case GET: {
				...
				if (dataMapping != null) {
					...
					for (Map.Entry<String, Object> entry : dataMapping
							.entrySet()) {
				...
				try {
					...
					if (connection.getApi().equalsIgnoreCase(
							RESTConstants.APPSTACK)) {
						if (restClient.ExecuteHEAD(
								login_app_url(query[position]),
							...
							url.insert(query[position].indexOf('/', 10) + 1,
									RESTConstants.APPSTACK_PROXY).toString();
							query[position] = url.toString();
						}
						response = restClient.ExecuteGet(null, query[position]);
					} else {
						params.add(new BasicNameValuePair("offset", String
								.valueOf(offset)));
						params.add(new BasicNameValuePair("limit", String
								.valueOf(limit)));
						offset += limit;
						response = restClient.ExecuteGet(params,
								query[position]);
				...
				if (columnNameMapping != null) {
					int remaining = this.fillColumnMappings(response,
							columnNameMapping);
					if (remaining >= limit) {
						continue;
					}
			...
			case POST: {
				...
				for (Map.Entry<String, Object> entry : param.entrySet()) {
					Object jsonobj = entry.getValue();
					 JSONObject jb=RESTJsonExtract.parse(jsonobj);
					jsonlist.add(jb);
				...
				try {
					response = restClient
							.ExecutePost(jsonlist, query[position]);
				} catch (Exception ex) {
					ex.printStackTrace();
					throw new OdaException("Data from the Server has Exception");
				}
				...
				Map<String, String> datamappingstring = dataMappingList
						.get(position);
				for (Map.Entry<String, String> entry : datamappingstring
						.entrySet()) {
					dataMapping.put(entry.getValue(),
							returnstring.get(entry.getKey()));
				}
				break;
			}
			}
					...
					while (i.hasNext()) {
						...
						if (e.getKey().equals("resources")) {
							...
							for (int j = 0; j < jsonresourceobj.size(); j++) {
								...
								siqList.createRow();
								JSONObject obj1 = jsonresourceobj
										.getJSONObject(j);
								arrayresponse.put(this.getArrayresponsecount(),
										obj1.toString());
	...
	public boolean extraction(String parent, String JSONString,
			boolean parentcheck, boolean levelcheck) throws JSONException {
		JSONObject jsonobj=RESTJsonExtract.parse(parent);
		Iterator i = jsonobj.entrySet().iterator();
		while (i.hasNext()) {
			Map.Entry e = (Map.Entry) i.next();

			if (e.getValue().getClass().getName() == "org.apache.commons.json.JSONArray"
					|| e.getValue().getClass().getName() == "org.apache.commons.json.JSONObject") {
				...
				if (columncheck(parent, this.siqList.getColumnlist())) {
					...
					case "array":
						...
						if (arrayrunning.get(parent).equals(
								arraysize.get(parent) - 1)) {
							extraction(parent,
									jarray.getString(arrayrunning.get(parent))
											.toString(), true, true);

						} else {
							arrayrunning.put(parent,
									arrayrunning.get(parent) + 1);
							extraction(parent,
									jarray.getString(arrayrunning.get(parent))
											.toString(), true, true);
						}
				...
				for (int j = 0; j < columnMappingList.size(); j++) {
					ColumnNameMapping columnNameMapping = columnMappingList
							.get(j);
					if (columnNameMapping != null) {

						int position = this.siqList.getColumnlist().indexOf(
								e.getKey());
						if (position != -1)
							if (columnNameMapping.getDestinationKey().equals(
									e.getKey())
									&& columnMapping.get(e.getValue()) != null) {
								position = this.siqList.getColumnlist()
										.indexOf(
												columnNameMapping
														.getSourceValue());
								...
								if (this.siqList
										.getDatatype()
										.get(this.siqList.getColumnlist()
												.indexOf(e.getKey()))
										.equalsIgnoreCase("uuid")) {
									this.siqList.addObj(e.getValue().toString()
				...
				if (!check) {
					...
					if (parentparam != null) {
						position = this.siqList.getColumnlist().indexOf(
								parentparam);
						if (position != -1) {
							if (this.siqList
									.getDatatype()
									.get(this.siqList.getColumnlist().indexOf(
											parentparam))
									.equalsIgnoreCase("uuid")) {
								this.siqList.addObj(e.getValue().toString()
										.replaceAll("-", "").toUpperCase(),
		...
		if (!levelcheck && pcheck) {
			siqList.addtoRowList();
			siqList.createRow();
			extraction(null, arrayresponse.get(this.getArrayresponsecount()),
					false, false);
	...
	public boolean columncheck(String item, List<String> columnlist) {
		List<String> child = this.getRESTlist().getComplicatedcolumns()
				.get(item);
